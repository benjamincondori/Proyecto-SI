<div>
    ...
    @else
        ...
        <div class="mb-2 d-flex justify-content-between">
            ...
            <div class="form-group w-50 d-flex">
                @if (verificarPermiso('Paquete_Buscar'))
                    ...
                    @if ($paquetes->count())
                        @foreach ($paquetes as $paquete)
                            <tr class="text-wrap text-center">
                                ...
                                <td class="align-middle text-left">{{ $paquete->nombre }}</td>
                                <td class="align-middle text-left">{{ $paquete->descripcion }}</td>
                                <td class="align-middle text-nowrap">
                                    ...
                                    <button type="button"title="Eliminar"
                                        wire:click="$emit('eliminarRegistro', {{ $paquete->id }}, {{ $this->verificarPermiso }})"
        ...
        <div class="d-flex justify-content-end justify-content-sm-between pt-3 pb-0">
            <div class="text-muted d-none d-sm-block pt-1">
                Mostrando del {{ ($paquetes->firstItem()) ? $paquetes->firstItem() : 0 }} al {{ ($paquetes->lastItem()) ? $paquetes->lastItem() : 0 }} de {{ $paquetes->total() }} registros
            </div>
    ...
    @push('js')
        ...
        <script>
            ...
            livewire.on('eliminarRegistro', function(paqueteId, tienePermiso) {
                if (tienePermiso) {
                    Swal.fire({
                        title: '¿Está seguro?',
