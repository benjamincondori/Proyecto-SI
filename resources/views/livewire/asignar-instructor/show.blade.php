<div>
    ...
    @if ($vistaCrear)
        <livewire:asignar-instructor.create>
    @elseif ($vistaEditar)
        <livewire:asignar-instructor.edit>
    @else
            ...
            <table class="table table-bordered table-hover mb-0">
                <thead class="bg-dark text-white text-nowrap">
                    <tr style="cursor: pointer">
                        <th scope="col" style="width: 160px;" wire:click="order('id_entrenador')">Id Entrenador
                            @if ($sort == 'id_entrenador')
                                @if ($direction == 'asc')
                        ...
                        <th scope="col" wire:click="order('id_entrenador')">Entrenador
                            @if ($sort == 'id_entrenador')
                                @if ($direction == 'asc')
                                    <i class="fas fa-sort-alpha-down float-md-right" style="margin-top: 4px"></i>
                                @else
                        ...
                        <th scope="col" wire:click="order('id_disciplina')">Disciplina
                            @if ($sort == 'id_disciplina')
                                @if ($direction == 'asc')
                                    <i class="fas fa-sort-alpha-down float-md-right" style="margin-top: 4px"></i>
                                @else
                                    <i class="fas fa-sort-alpha-up float-md-right" style="margin-top: 4px"></i> 
    ...
    @push('js')
        ...
        <script>
            ...
            livewire.on('eliminarRegistro', function(entrenadorId, disciplinaId, tienePermiso) {
                if (tienePermiso) {
                    Swal.fire({
                        title: '¿Está seguro?',
                        ...
                        if (result.isConfirmed) {
                            ...
                            livewire.emitTo('asignar-instructor.show', 'eliminarAsignacion', entrenadorId, disciplinaId);
