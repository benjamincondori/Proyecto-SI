<div>
    ...
    @else
        <div class="mb-2 d-flex justify-content-between">
            ...
            <div class="form-group w-50 d-flex">
                @if (verificarPermiso('Seccion_Buscar'))   
            ...
            <table class="table table-bordered table-hover mb-0">
                <thead class="bg-dark text-white text-nowrap">
                    <tr style="cursor: pointer">
                        ...
                        <th scope="col" style="width: 120px;" wire:click="order('capacidad')">Capacidad
                            @if ($sort == 'capacidad')
                                @if ($direction == 'asc')
                                    <i class="fas fa-sort-alpha-down float-md-right" style="margin-top: 4px"></i>
                                @else
                    ...
                    @if ($secciones->count())
                        @foreach ($secciones as $seccion)
                            <tr class="text-wrap text-center">
                                ...
                                <td class="align-middle text-left">{{ $seccion->nombre }}</td>
                                <td class="align-middle text-left">{{ $seccion->descripcion }}</td>
                                ...
                                <td class="align-middle text-nowrap">
                                    ...
                                    <button type="button" title="Eliminar" wire:click="$emit('eliminarRegistro', {{ $seccion->id }}, {{ $this->verificarPermiso }})" class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i></button>
                                </td>
                            </tr>
                        @endforeach
                    @else
        ...
        <div class="d-flex justify-content-end justify-content-sm-between pt-3 pb-0">
            <div class="text-muted d-none d-sm-block pt-1">
                Mostrando del {{ ($secciones->firstItem()) ? $secciones->firstItem() : 0 }} al {{ ($secciones->lastItem()) ? $secciones->lastItem() : 0 }} de {{ $secciones->total() }} registros
            </div>
    ...
    @push('js')
        ...
        <script>
            ...
            livewire.on('eliminarRegistro', function(seccionId, tienePermiso) {
                if (tienePermiso) {
                    Swal.fire({
                        title: '¿Está seguro?',
